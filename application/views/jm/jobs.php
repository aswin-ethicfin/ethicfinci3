            ...
            <div class="card my-4">
                ...
                <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                    <div class="bg-gradient-primary shadow-primary border-radius-lg pt-4 pb-3 d-flex justify-content-between align-items-center">
                        ...
                        <div class="pe-3">
                            <a href="<?= base_url('jm/newjob'); ?>" class="btn btn-sm btn-outline-white mb-0">
                ...
                <div class="card-body px-4 py-3">
                    ...
                    <!-- Filter Form -->
                    <form method="get" action="<?= base_url('jm/listjobs') ?>" class="row g-1 mb-2">
                        <div class="col-md-1">
                            ...
                            <tbody>
                                ...
                                ?>
                                        <tr>
                                            ...
                                            <td>
                                                <?php if ($job['job_status'] != 10): ?>
                                                    <a href="javascript:void(0);"
                                                        class="btn btn-link text-success px-3 mb-0"
                                                        onclick="convertToInvoice('<?= $job['job_id']; ?>', this)"
                                                        title="Convert To Invoice">
                                                        <i class="fas fa-file-invoice-dollar"></i>
                                                    </a>
                                                <?php endif; ?>
                                                <a href="<?= base_url('jm/jobview?id=' . $job['job_id']); ?>"
                                                    class="btn btn-link text-info px-3 mb-0" title="View">
                                                    <i class="fa fa-eye"></i>
                                                </a>
                            ...
                            <div class="d-flex justify-content-between align-items-center mt-4">
                                <ul class="pagination mb-0">
                                    ...
                                    <li class="page-item <?= ($page <= 1) ? 'disabled' : '' ?>">
                                        <a class="page-link" href="<?= $baseUrl . '?page=' . $prevPage . $suffix ?>">
                                            <i class="fa fa-arrow-circle-left"></i>
                                        </a>
                                    </li>

                                    <!-- First page + ellipsis -->
                                    <?php if ($page > $windowSize + 1): ?>
                                        <li class="page-item">
                                            <a class="page-link" href="<?= $baseUrl . '?page=1' . $suffix ?>">1</a>
                                        </li>
                                        <?php if ($page > $windowSize + 2): ?>
                                    ...
                                    ?>
                                        <li class="page-item <?= ($i == $page) ? 'active' : '' ?>">
                                            <a class="page-link" href="<?= $baseUrl . '?page=' . $i . $suffix ?>">
                                                <?= $i ?>
                                            </a>
                                        </li>
                                    <?php endfor; ?>
                                    ...
                                    <!-- Last page + ellipsis -->
                                    <?php if ($page < $totalPages - $windowSize): ?>
                                        <?php if ($page < $totalPages - $windowSize - 1): ?>
                                            <li class="page-item disabled">
                                                <span class="page-link">…</span>
                                            </li>
                                        <?php endif; ?>
                                        <li class="page-item">
                                            <a class="page-link" href="<?= $baseUrl . '?page=' . $totalPages . $suffix ?>">
                                                <?= $totalPages ?>
                                            </a>
                                        </li>
                                    <?php endif; ?>
                                    ...
                                    <li class="page-item <?= ($page >= $totalPages) ? 'disabled' : '' ?>">
                                        <a class="page-link" href="<?= $baseUrl . '?page=' . $nextPage . $suffix ?>">
                                ...
                                <small class="text-muted">
                                    Showing <?= ($start + 1) ?>–<?= min($start + $perPage, $totalRows) ?>
                                    of <?= $totalRows ?> &nbsp;·&nbsp;
...
<script>
    ...
    $(document).on('change', '.status-select-premium', function() {
        ...
        $.ajax({
            url: "<?= base_url('jm/updatejobstatus') ?>",
            type: "POST",
            data: {
                job_id: jobId,
                job_status: status
    ...
    function convertToInvoice(jobId) {
        ...
        if (!confirm("Convert this job to an invoice? This will create a new invoice.")) return;

        $.ajax({
            url: "<?= base_url('jm/converttoinvoice') ?>",
            type: "POST",
            data: {
                job_id: jobId
            },
            ...
            success: function(res) {
                if (res.status === "success") {
                    ...
                    setTimeout(() => {
                        window.location.href = res.redirect_url; // go to invoice view
                    }, 1500);
