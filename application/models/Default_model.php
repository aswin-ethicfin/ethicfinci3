...
class Default_model extends CI_Model
{
	...
	public function generateInvoiceReference()
	{
		...
		$result = $this->db->query(
			"SELECT COUNT(*) as total FROM tbl_invoice WHERE YEAR(inv_date) = ?",
			[$year]
		)->row_array();
		$count = (int)$result['total'] + 1;
		return 'INV-' . $year . '-' . str_pad($count, 5, '0', STR_PAD_LEFT);
	}
	public function generateInvoiceNumber()
	{
		$result = $this->db->query(
			"SELECT MAX(CAST(SUBSTRING_INDEX(inv_no, '-', -1) AS UNSIGNED)) as max_no 
         FROM tbl_invoice"
		)->row_array();
		$next = (int)$result['max_no'] + 1;
